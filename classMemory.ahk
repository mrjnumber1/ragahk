...
class _ClassMemory
{
    ...
    { 
        For index, _offset in offsets
            address := this.Read(address, this.ptrType) + _offset 
    ...
    */  
    ...
    EnumProcessModulesEx(byRef lphModule, dwFilterFlag := 0x03)
    {
		...
		reqSize := 0
        _size := VarSetCapacity(lphModule, 4)
        loop 
        {
            ...
            if ErrorLevel
                return 0
            else if (_size >= reqSize)
                break
            else _size := VarSetCapacity(lphModule, reqSize)  
        }
    ...
    */
    ...
    patternScan(startAddress, sizeOfRegionBytes, byRef patternMask, byRef needleBuffer)
    {
        if !this.readRaw(startAddress, buffer, sizeOfRegionBytes)
            return -1      
        if (_offset := this.bufferScanForMaskedPattern(&buffer, sizeOfRegionBytes, patternMask, &needleBuffer)) >= 0
            return startAddress + _offset 
        else return 0
    }
